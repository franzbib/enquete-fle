        ...
        <>
            ...
            <div className="mt-5 grid gap-3">
              ...
              <button
                ...
                disabled={
                  !isAvailable ||
                  isSolved ||
                  orderedAnswer.length !== answer.events.length ||
                  orderedAnswer.some((eventId) => !eventId)
                }
          ...
          {answer.kind === 'single-choice' ? (
            <div className="mt-5 flex flex-col gap-3">
              {answer.options.map((option) => (
                <label className="choice-card text-sm" key={option.id}>
                  <input
                    className="mr-2"
                    disabled={!isAvailable || isSolved}
                    type="radio"
          ...
          {answer.kind === 'case-file-contradiction' ? (
            <div className="mt-5 grid gap-5">
              <div>
                ...
                <div className="mt-3 grid gap-2">
                  {answer.caseFileItems.map((item) => {
                    ...
                    return (
                      <label className="choice-card text-sm" key={item.documentId}>
                        <input
                          className="mr-2"
                          disabled={
                            !isAvailable ||
              ...
              {localFeedback ? (
                <p className="info-strip text-sm leading-6">{localFeedback}</p>
              ) : null}
                  ...
                  {answer.interpretationOptions.map((option) => (
                    <label className="choice-card text-sm" key={option.id}>
                      <input
                        ...
                        onChange={(event) => setInterpretationOptionId(event.target.value)}
                      />
                      {option.label}
                    </label>
                  ))}
